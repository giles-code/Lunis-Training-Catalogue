...
// Shared behaviour for an audience-locked course listing page (e.g. Supervisors-only,
// Firms-only). Call this from a page's $w.onReady with the fixed `audience` value that
// page should show. Unlike catalogue.js, there is no audienceDropdown here — the audience
// is fixed by which page you're on.
export async function initAudienceCatalogue(fixedAudience) {
  let allCourses = [];
  let filteredCourses = [];
  let view = 'cards';
  ...
  try {
    ...
    allCourses = result.courses
      .filter((course) => (course.audience || []).includes(fixedAudience))
  ...
  function wireFilters() {
    ...
    filterSelectors().forEach((selector) => $w(selector).onChange(applyFilters));

    $w('#clearFiltersButton').onClick(() => {
      $w('#searchInput').value = '';
      filterSelectors().forEach((selector) => { $w(selector).selectedIndex = 0; });
      applyFilters();
    });
  }
